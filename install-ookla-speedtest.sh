...
out = {
    ...
    # Non-standard extensions to the legacy schema: Ookla's CLI reports
    # jitter (ping variance -- the real signal for bufferbloat, since a
    # line can have low average ping but still spike badly under load)
    # and packet loss directly, which the classic speedtest-cli schema
    # has no equivalent for. netmon's models treat these as optional.
